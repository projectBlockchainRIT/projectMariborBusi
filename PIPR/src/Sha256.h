...
// Minimal header-only SHA256 implementation (picosha2, public domain).
#include <array>
#include <cstdint>
#include <iomanip>
#include <sstream>
...
inline void transform(const uint8_t* chunk, uint32_t* h) {
    uint32_t w[64];
    ...
    for (int i = 0; i < 64; i++) {
        uint32_t t1 = h_val + bsig1(e) + ch(e, f, g) + k[i] + w[i];
...
inline std::array<uint8_t, 32> hash(const std::vector<uint8_t>& data) {
...
inline std::string sha256(const std::string& input) {
    ...
    auto bytes = sha256_internal::hash(data);
    return sha256_internal::to_hex(bytes);
}
