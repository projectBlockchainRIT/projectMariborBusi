  ...
  useEffect(() => {
    ...
    const fetchOccupancyData = async () => {
      try {
        ...
        const response = await fetch(
          getApiUrl(`occupancy/line/${selectedLineId}/date/${currentDate}`),
          {
            headers: {
              Accept: "application/json",
            },
        ...
        if (!response.ok) {
          if (response.status === 404) {
            setError(
              `No passenger data available for ${selectedRouteName} on ${formatDateForDisplay(currentDate)}`,
            );
            setLoading(false);
            return;
          }
        ...
        // Sort data by hour
        newLineData.data.sort(
          (a, b) =>
            parseInt(a.hour.replace("h", "")) -
            parseInt(b.hour.replace("h", "")),
        );

        // Update line data, replacing previous data for this line
        setLineData((prevData) => {
          const otherLines = prevData.filter(
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load occupancy data",
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-8 bg-white dark:bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 dark:border-slate-700/50">
      ...
      <div className="relative h-[500px] w-full">
        ...
        <div className="absolute inset-0 px-4">
          ...
          {loading ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-gray-500 dark:text-gray-400">
                No data available for {selectedRouteName} on{" "}
            ...
            <div className="relative h-full">
              ...
              >
                ...
                {selectedLineData.length > 1 && (
                  <path
                    d={`M 20 ${600 - (selectedLineData[0].density / 100) * 480} ${selectedLineData
                      .slice(1)
                      .map((point, i) => {
                        const x =
                          20 + ((i + 1) / (selectedLineData.length - 1)) * 960;
      ...
      <div className="mt-6 text-sm text-gray-600 dark:text-gray-400 flex justify-between items-center">
        <div>
          <span className="font-medium">Note:</span> Graph shows passenger
          density as a percentage of maximum capacity
        </div>
