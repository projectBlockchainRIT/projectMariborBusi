...
export default function OccupancyController({
  ...
  const fetchOccupancyData = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch occupancy data: ${response.status} ${response.statusText}`,
        );
      }
      const data = await response.json();
      console.log(
        `[API] Occupancy response for line ${lineId}, date ${date}, hour ${hour}:`,
        data,
      );
      ...
      if (Array.isArray(data?.data)) {
        ...
        const found = data.data.find(
          (entry: any) => entry.Time && entry.Time.includes(`T${hourStr}:`),
        );
      ...
      console.log(
        `[API] Error for line ${lineId}, date ${date}, hour ${hour}:`,
        error,
      );
      return null;
    }
  ...
  const handleRouteClick = async (route: Route) => {
    ...
    if (mapInstance) {
      await drawRouteOnMap(mapInstance, route, {
  ...
  const animatedHour =
    isPlaying && currentHourIdx !== null ? PRESET_HOURS[currentHourIdx] : null;
  ...
  useEffect(() => {
    ...
    if (route) {
      drawRouteOnMap(mapInstance, route, {
        setStatus: (msg: string) => console.log(msg),
      });
    }
  ...
  return (
    <div
      className={`h-full w-80 shadow-lg transition-colors duration-200 ${isDarkMode ? "bg-gray-800 text-gray-200" : "bg-white text-gray-700"}`}
    >
      <div className="h-full flex flex-col">
        <div
          className={`p-4 border-b transition-colors duration-200 ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
        >
            ...
            <>
              ...
              <div className="flex flex-col gap-2 mb-4">
                {PRESET_HOURS.map((hour, idx) => (
                  ...
                  >
                    {loadingOccupancy
                      ? "Loading..."
                      : occupancyData[displayDate]?.[hour] !== undefined
                        ? `Hour ${hour}: ${occupancyData[displayDate]?.[hour]}`
                        : `Hour ${hour}: -`}
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <button
                  onClick={handlePlay}
                  disabled={
                    isPlaying || loadingOccupancy || PRESET_HOURS.length === 0
                  }
                ...
                <Slider
                  ...
                  onChange={(_, v) =>
                    setAnimationSpeed(typeof v === "number" ? v : 1)
                  }
