...
def create_training_pairs(df):
    ...
    X = [lat, lon, signal_strength] za vse uporabnike (weighted by type)
    ...
    if is_multibus:
        ...
        for timestamp, group in df.groupby('timestamp'):
            ...
            bus_lat = on_bus_users['bus_lat'].iloc[0] if 'bus_lat' in on_bus_users.columns else on_bus_users['lat'].mean()
            bus_lon = on_bus_users['bus_lon'].iloc[0] if 'bus_lon' in on_bus_users.columns else on_bus_users['lon'].mean()
...
def main():
    ...
    # Try multibus files first
    csv_files = glob.glob(f"{DATA_FOLDER}/*_multibus_*.csv")
    if not csv_files:
