...
def main():
    ...
    if not Path("routes.json").exists():
        print(f"{Colors.FAIL}routes.json ne obstaja!{Colors.ENDC}")
        sys.exit(1)
    ...
    print(f"   Generator: {'Python' if use_generator else 'C++'}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n\n{Colors.WARNING}Pipeline prekinjen s strani uporabnika{Colors.ENDC}")
        sys.exit(0)
    except Exception as e:
